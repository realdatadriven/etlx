...
table: DDBPTEST
load_conn: "duckdb:"
load_before_sql: "ATTACH 'ducklake:sqlite:database/parallel_dl_metadata.sqlite' AS DB (DATA_PATH 'database/parallel_dl_metadata/')"
load_sql: |
  CREATE OR REPLACE TABLE DB."<table>" AS SELECT  v1.x % 1000 AS category,  COUNT(*), APPROX_COUNT_DISTINCT(v1.y), AVG(v1.z)
...
connection: "duckdb:"
before_sql:
  - "ATTACH 'database/parallel_logs.db' AS DB (TYPE SQLITE)"
  - "USE DB;"
  - LOAD json
  - "get_dyn_queries[create_columns_missing](ATTACH 'database/parallel_logs.db' AS DB (TYPE SQLITE),DETACH DB)"
