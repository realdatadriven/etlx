...
func (etlx *ETLX) GetDB(conn string) (db.DBInterface, error) {
	...
	case "quack":
		...
		if err == nil {
			...
			disableSSL, ok := params["disable_ssl"]
			if !ok {
				disableSSL, ok = params["DISABLE_SSL"]
				if !ok {
					disableSSL = "false"
			...
			host, port, params, err := etlx.ParseQuackDSN(_dsn)
			if err != nil {
				return nil, fmt.Errorf("%s ParseQuackDSN: %s", driver, err)
			}
			if host == "" {
				host = "localhost"
			}
...
func (etlx *ETLX) containsAny(slice []any, element any) bool {
	for _, v := range slice {
		if v == element {
			return true
		}
...
func (etlx *ETLX) RunETL(dateRef []time.Time, conf map[string]any, extraConf map[string]any, keys ...string) ([]map[string]any, error) {
	...
	ELTRunner := func(metadata map[string]any, itemKey string, item map[string]any) error {
		...
		start2 := time.Now().In(etlx.TimeZone)
